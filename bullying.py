...
from .. import (
    loader, utils, validators, database)
...
@loader.module(name="bull", author="HotDrify", version=1)
class BullMod(loader.Module):
    """Унижатор, унижает всех!"""
    def __init__(self):
        self.config = Config(
            ConfigValue(
                'addbull_text',
                'давай поговорим, чурка ебанная)',
                'давай поговорим, чурка ебанная)',
                validators.String(),
            ),
            ConfigValue(
                'rmbull_text',
                'идешь нахуй, слитая немощь.',
                'идешь нахуй, слитая немощь.',
                validators.String(),
            ),
            ConfigValue(
    ...
    async def addbull_cmd(self, app: Client, message: types.Message):
        """я Владик, ой. Угомонить пидора."""
        reply = message.reply_to_message
        if reply.from_user.id in self.db.get("bull", "users", []):
            return await utils.answer(
                message,
                "❌ заебал, я его и так унижаю."
            )
        ...
        if reply:
            if reply.from_user.id:
                await utils.answer(
                  message,
                  self.config["addbull_text"]
                )
                users = self.db.get("bull", "users", [])
                users.append(reply.from_user.id)
                self.db.set("bull", "users", users)
                await utils.answer(
                  message,
                  self.db.get("bull", "users", [])
                )
        else:
            await utils.answer(
              message,
    ...
    async def rmbull_cmd(self, app: Client, message: types.Message):
        reply = message.reply_to_message
        if not reply.from_user.id in self.db.get("bull", "users", []):
            return await utils.answer(
              message,
              "❌ эта слитая не попадала пол мои словечки."
            )
